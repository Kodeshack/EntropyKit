...
class RoomService {
    static func send<M: MessageJSON>(message: M, to roomID: String, encrypted: Bool, account: Account, database: Database, completionHandler: @escaping (Result<Message>) -> Void) {
    ...
    static func sendMedia(filename: String, data: Data, eventType: Message.MessageType = .file, info: FileMessageJSON.Info, encrypted: Bool = false, roomID: RoomID, account: Account, database: Database, completionHandler: @escaping (Result<Message>) -> Void) {
        let payload: Data
        ...
        if encrypted {
            ...
            guard let encryption = encryptionResult.value else { return completionHandler(.Error(encryptionResult.error!)) }
        ...
        MatrixAPI.default.upload(filename: filename, mimeType: mimeType, data: payload, accessToken: account.accessToken) { uploadResult in
            ...
            guard let contentURI = uploadResult.value else { return completionHandler(.Error(uploadResult.error!)) }
            if var encryptionInfo = encryptionInfo, let mxcURL = URL(string: contentURI) {
                ...
                return completionHandler(.Error(MatrixAPI.APIError.invalidFile))
            }
    ...
    private static func sendRequest<M: MessageJSON>(account: Account, roomID: String, body: JSONEncodable, eventType: Event.EventsType, message: M, database: Database, completionHandler: @escaping (Result<Message>) -> Void = { _ in }) {
