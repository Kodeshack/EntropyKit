...
class AttachmentEncryptionTests: XCTestCase {
    func testEncrypt() {
        ...
        let encryptedAttachment = try! AttachmentEncryption.encrypt(plainData: file, mimeType: "image/png").dematerialize()
        ...
        let decryptedData = try! AttachmentEncryption.decrypt(ciphertext: encryptedAttachment.ciphertext, info: attachmentCryptoInfo).dematerialize()
