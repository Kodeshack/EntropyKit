...
extension MatrixAPI {
    ...
    func downloadImage(mxcURL: URL, cryptoInfo: Attachment.Info.CryptoInfo? = nil, queue: DispatchQueue = DispatchQueue.main, completionHandler: @escaping (Swift.Result<Image, Error>) -> Void) {
    ...
    func downloadFile(mxcURL: URL, cryptoInfo: Attachment.Info.CryptoInfo? = nil, queue: DispatchQueue = DispatchQueue.main, completionHandler: @escaping (Swift.Result<Data, Error>) -> Void) {
        createRequest(url: getMediaURL(mxcURL)).responseData(queue: queue) { response in
            completionHandler(Swift.Result {
                ...
                if let cryptoInfo = cryptoInfo {
                    data = try AttachmentEncryption.decrypt(ciphertext: data, info: cryptoInfo).get()
                }
    ...
    func upload(filename: String, mimeType: String, data: Data, accessToken: String, queue: DispatchQueue = DispatchQueue.main, completionHandler: @escaping (Swift.Result<String, Error>) -> Void) {
        ...
        sessionManager.upload(data, to: url, method: .post, headers: headers)
            ...
            .responseJSON(queue: queue) { response in
                ...
                guard let uploadResponse = uploadResponseResult.success else {
                    completionHandler(.failure(uploadResponseResult.failure!))
                    return
                }

                completionHandler(.success(uploadResponse.contentURI))
            }
    }
}
...
extension MatrixAPI {
    ...
    func getAvatarURL(userID: UserID, queue: DispatchQueue = DispatchQueue.main, completionHandler: @escaping (Swift.Result<AvatarURLResponse, Error>) -> Void) {
    ...
    ///   - queue: Dispatch queue on which to execute the callback. Defaults to main queue.
    ///   - completionHandler: Called with the resulting NSImage instance (or Error) when the request has ended (whether successfully or unsuccessfully).
    func downloadAvatar(mxcURL: URL, queue: DispatchQueue = DispatchQueue.main, completionHandler: @escaping (Swift.Result<Image, Error>) -> Void) {
...
extension MatrixAPI {
    ...
    func login(username: String, password: String, queue: DispatchQueue = DispatchQueue.main, completionHandler: @escaping (Swift.Result<LoginResponse, Error>) -> Void) {
...
extension MatrixAPI {
    ...
    func send(body: JSONEncodable, eventType: Event.EventsType, to roomID: RoomID, accessToken: String, transactionID: UInt, queue: DispatchQueue = DispatchQueue.main, completionHandler: @escaping (Swift.Result<EventResponse, Error>) -> Void) {
...
extension MatrixAPI {
    ...
    ///   - queue: Dispatch queue on which to execute the callback.
    ///   - completionHandler: Called with the response (or error) when the request has ended (whether successfully or unsuccessfully).
    func sync(timeout: Int, nextBatch: String = "", accessToken: String, queue: DispatchQueue, completionHandler: @escaping (Swift.Result<SyncResponse, Error>) -> Void = { _ in }) {
...
extension MatrixAPI {
    ...
    ///   - queue: Dispatch queue on which to execute the callback.
    ///   - completionHandler: Called with the response (or error) when the request has ended (whether successfully or unsuccessfully).
    func claimKeys(accessToken: String, keysClaimRequest: KeysClaimRequest, queue: DispatchQueue, completionHandler: @escaping (Swift.Result<KeysClaimResponse, Error>) -> Void) {
        createRequest(for: .claimKeys, accessToken: accessToken, method: .post, payload: keysClaimRequest)
            .responseData(queue: queue) { response in
                completionHandler(Swift.Result {
                    ...
                    return try KeysClaimResponse.decode(data).get()
                })
            }
    }
    ...
    func announceDevice(accessToken: String, transactionID: UInt, deviceAnnouncementRequest: DeviceAnnouncementRequest, completionHandler: @escaping (Swift.Result<(), Error>) -> Void) {
        createRequest(for: .newDevice, accessToken: accessToken, transactionID: transactionID, method: .put, payload: deviceAnnouncementRequest)
            .responseData { response in
                completionHandler(Swift.Result {
                    let data = try response.result.unwrap()
                    _ = try EmptyJSON.decode(data).get()
                })
            }
    }
    ...
    ///   - queue: Dispatch queue on which to execute the callback.
    ///   - completionHandler: Called with the response (or error) when the request has ended (whether successfully or unsuccessfully).
    func uploadKeys(accessToken: String, deviceID: String, keysUploadRequest: KeysUploadRequest?, queue: DispatchQueue, completionHandler: @escaping (Swift.Result<UInt, Error>) -> Void) {
        createRequest(for: .keyUpload, path: deviceID, accessToken: accessToken, method: .post, payload: keysUploadRequest ?? EmptyJSON())
            .responseData(queue: queue) { response in
                completionHandler(Swift.Result {
                    let data = try response.result.unwrap()

                    let keysUploadReponse = try KeysUploadResponse.decode(data).get()
    ...
    ///   - queue: Dispatch queue on which to execute the callback.
    ///   - completionHandler: Called with the response (or error) when the request has ended (whether successfully or unsuccessfully).
    func queryKeys(accessToken: String, keysQueryRequest: KeysQueryRequest, queue: DispatchQueue, completionHandler: @escaping (Swift.Result<KeysQueryResponse, Error>) -> Void) {
        createRequest(for: .queryKeys, accessToken: accessToken, method: .post, payload: keysQueryRequest)
            .responseData(queue: queue) { response in
                completionHandler(Swift.Result {
                    let data = try response.result.unwrap()
                    return try KeysQueryResponse.decode(data).get()
                })
            }
    }
    ...
    ///   - queue: Dispatch queue on which to execute the callback.
    ///   - completionHandler: Called with the response (or error) when the request has ended (whether successfully or unsuccessfully).
    func publishSession(accessToken: String, transactionID: UInt, publishRoomKeysRequest: PublishRoomKeysRequest, queue: DispatchQueue, completionHandler: @escaping (Swift.Result<Void, Error>) -> Void) {
...
extension MatrixAPI {
    ...
    ///   - queue: Dispatch queue on which to execute the callback.
    ///   - completionHandler: Called with the response (or error) when the request has ended (whether successfully or unsuccessfully).
    func requestLinkPreviewInfo(for url: URL, at timestamp: UInt? = nil, accessToken: String, queue: DispatchQueue = DispatchQueue.main, completionHandler: @escaping (Swift.Result<LinkPreviewResponse, Error>) -> Void) {
        ...
        createRequest(in: .media, for: .previewURL, accessToken: accessToken, parameters: parameters)
            .responseData(queue: queue) { response in
                let infoResult = Swift.Result<LinkPreviewResponse, Error> {
                    let data = try response.result.unwrap()
                    return try LinkPreviewResponse.decode(data).get()
                }

                guard var info = infoResult.success else {
                    completionHandler(.failure(infoResult.failure!))
                    return
                }
